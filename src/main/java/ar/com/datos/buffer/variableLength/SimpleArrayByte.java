 ...
 *
 */
public class SimpleArrayByte implements ArrayByte {
	private byte[] miArr;
	private Integer start;
	private Integer length;
	public SimpleArrayByte(byte[] miArr) {
		this.miArr = miArr;
		this.start = 0;
		this.length = miArr.length;
	}
	protected SimpleArrayByte(byte[] miArr, Integer start, Integer length) {
		this(miArr);
		this.start = start;
		this.length = length;
	}
	...
	public SimpleArrayByte getRightSubArray(Integer beginIndex) {
		if (beginIndex > getLength()) throw new IndexOutOfBoundsException();
		return new SimpleArrayByte(this.miArr, this.start + beginIndex, this.getLength() - beginIndex);
	}
	public SimpleArrayByte getLeftSubArray(Integer endIndex) {
		if (endIndex > getLength()) throw new IndexOutOfBoundsException();
		return new SimpleArrayByte(this.miArr, this.start, endIndex);
	}
	public ArrayByte getSubArray(Integer beginIndex, Integer endIndex) {
		...
		return new SimpleArrayByte(this.miArr, this.start + beginIndex, endIndex - beginIndex);
	}
	@Override
	public int hashCode() {
		final int prime = 31;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SimpleArrayByte other = (SimpleArrayByte) obj;
		if (length == null) {
